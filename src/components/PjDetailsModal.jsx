...
function PjDetailsModal({ 
  ...
  const handleChange = (campo, valor) => {
    if (campo === 'wones' && !/^\d*$/.test(valor)) return; // solo números
  ...
  return (
    <div className="modal-overlay flex items-center justify-center min-h-screen bg-black bg-opacity-40 z-50 p-1 sm:p-4">
      <div className="modal-content max-w-[1800px] w-full mx-0 sm:mx-8 max-h-[98vh] min-h-[80vh] overflow-y-auto p-1 sm:p-8 rounded-2xl shadow-2xl bg-white relative">
        ...
        <div className="card-header flex flex-col sm:flex-row items-center justify-between sticky top-0 bg-white z-10 border-b px-2 sm:px-0 py-2 sm:py-4 gap-2 sm:gap-0">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <div className="truncate">
              <h2 className="text-lg sm:text-2xl font-bold text-gray-800 truncate">{cuenta.nombre}</h2>
              <p className="text-xs sm:text-base text-gray-600 truncate">{cuenta.correo || 'Sin correo'}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="card-body p-2 sm:p-6">
          ...
          {cuenta.pejotas.length > 6 && !esBusqueda && (
          ...
          {(modalPj !== null || esBusqueda) && (
            <>
              ...
              <div className="w-full flex flex-col sm:flex-row items-center justify-between mb-4 sm:mb-6 gap-2 p-2 sm:p-3 rounded bg-blue-50 border border-blue-200 text-blue-900 text-xs sm:text-sm font-serif">
                <div className="truncate max-w-full"><b>Cuenta:</b> <span className="font-bold text-gray-800">{cuenta.nombre}</span></div>
                <div className="truncate max-w-full"><b>Correo:</b> <span className="font-mono text-blue-700">{cuenta.correo || 'Sin correo'}</span></div>
              </div>
